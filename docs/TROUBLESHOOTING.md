...
**Fix:** `delete_ingress_objects`'s script now polls `aws ec2 describe-security-groups --filters Name=tag:elbv2.k8s.aws/cluster,Values=<cluster>` for up to 2 minutes (24 x 5s) after the `kubectl delete ingress` calls succeed, before returning control to Terraform (which then destroys the Helm release). Deliberately best-effort (`|| true`, no hard failure) -- if the SG genuinely doesn't clear in that window, `terraform destroy` should still proceed rather than block on something that self-heals: `aws ec2 delete-security-group --group-id <id>` unblocks the eventual `DeleteVpc` immediately if it's still there by the time destroy gets that far.

**Lesson:** the original write-up here sounded plausible (EKS does auto-manage some things) and was never actually challenged until it recurred a third time and someone asked "but *why*, really?" -- a documented root cause that was never verified against real evidence (CloudTrail, in this case) is a guess wearing a citation's clothes.
