...
**Status:** all three fixed live (on the currently-running nodes and monitoring EC2) and in git (`modules/eks/node-user-data.sh.tftpl`, `modules/eks/iam.tf`, `modules/eks/variables.tf`, `modules/monitoring-ec2/user-data.sh.tftpl`, `main.tf`). Verified end-to-end after the live fix: `journalctl -u fluent-bit` showed real container log chunks being read and parsed; a full `terraform apply` picking up the templated fix is still needed to make this durable for the next fresh cluster (in progress as of this entry).
