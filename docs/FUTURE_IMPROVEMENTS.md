...
**Still open — full backend retirement.** Plan 4's own final task (deleting `backend/` and its K8s manifests once api-gateway proves out) was **intentionally not executed** — it's the one irreversible step in the whole series, and it was paused rather than run blind. Concretely:
- `backend/` and `k8s/base/` (the old monolith) are still on disk and still deployed via `k8s/argocd/application.yaml`, still serving `bookstore.<domain>` (frontend static assets).
- The ingress collision that used to block this is **resolved** — `k8s/base/ingress/ingress.yaml` no longer declares `api.bookstore.<domain>` at all, `k8s/services/api-gateway/base/ingress.yaml` is its sole owner, and this was verified live (`POST /books` without a token now correctly returns `401` from the gateway, not a silent write to the old backend). What's left of the cutover is purely the backend deletion itself, not a routing ambiguity.
