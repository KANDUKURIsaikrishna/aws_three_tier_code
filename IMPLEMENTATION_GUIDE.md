...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Bootstrap complete. Replace the empty strings in the versions.tf
backend block with the values below, then run: terraform init
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
### Step 2.2 — Fill in the backend block in versions.tf

The S3 backend block cannot use Terraform variables (it is parsed before variables are loaded), so you must edit `versions.tf` directly — once. Replace the empty `bucket` and `dynamodb_table` strings with the values printed by the bootstrap script:
...
> **All other project-specific values** (`domain`, `github_repo`) are in `terraform.tfvars`, which was generated by `scripts/configure.py` in Part 0. You do not need to edit `versions.tf` for those.

> Do this substitution exactly once. Every subsequent `terraform init` and `terraform apply` reuse the same S3 object.

### Step 2.3 — Reusing existing Terraform state (improvements branch)

If you already deployed from the `main` branch and want to reuse the same state on the `improvements` branch, skip Step 2.1 (the S3 bucket and DynamoDB table already exist). Fill in `versions.tf` with the same bucket name and table, then run the two state renames below before your first `terraform plan`. These are needed because the Terraform refactor renamed two resources:

```bash
terraform init

# ACM certificate resource was renamed: b17facebook_cert → this
terraform state mv \
  'module.acm.aws_acm_certificate.b17facebook_cert' \
  'module.acm.aws_acm_certificate.this'

# EBS CSI policy attachment moved from eks module to eks-addons module
terraform state mv \
  'module.eks.aws_iam_role_policy_attachment.node_ebs_csi' \
  'module.eks_addons.aws_iam_role_policy_attachment.ebs_csi_policy'

# Verify — should show: No changes. Your infrastructure matches the configuration.
terraform plan
```

Without these two `state mv` commands, Terraform would attempt to destroy and recreate the ACM certificate and briefly detach/reattach the EBS CSI IAM policy.
